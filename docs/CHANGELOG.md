...
1. **IMConfigPanel UI 组件**
   - 飞书/钉钉/企微/Slack 平台配置界面
   - Tab 切换支持
   - 密码显示/隐藏功能
...
- TaskEngine.cancel() 调用错误（调用 pause 而非 cancel）
- IPC 返回值自动包装导致前端解析失败
- 配置文件路径解析使用 process.cwd() 不稳定
- Agent 未初始化导致飞书消息处理失败
- UI 状态不更新问题

### 优化

- Agent 启动时预初始化，加快消息处理
- FeishuService 添加 cleanup 方法
- IPC 层添加 NO_WRAP_CHANNELS 机制
