...
${context.pageStructure ? `- 页面标题：${context.pageStructure.title || '无'}
- 页面链接列表（按出现顺序）:
${context.pageStructure.links?.slice(0, 30).map((l: any, i: number) => 
  `${i+1}. [${l.parentContext || 'unknown'}] ${l.text.substring(0, 60)} (index: ${l.index})`
).join('\n') || '无'}
- 主要容器：${context.pageStructure.containers?.slice(0, 10).join(', ') || '无'}
