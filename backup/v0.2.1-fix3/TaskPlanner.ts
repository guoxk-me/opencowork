...
- browser:click(selector, index?, textMatch?) - 点击元素
- browser:input(selector, text, clear?, textMatch?) - 输入文本
...
4. 描述要简洁明确，便于用户理解
5. 输出必须是有效的JSON格式
6. **选择器必须精确匹配目标元素**，使用以下策略：
   - 优先使用 ID (#id) 或唯一类名
   - 使用属性选择器：[type="submit"]、[name="search"]
   - 使用 textMatch 匹配按钮文本，如 "搜索"、"登录"
   - 避免使用模糊选择器如 "button"、"input"、"a"
   - index 仅作为最后手段
7. 对于搜索类操作：
   - 点击搜索按钮使用 textMatch 匹配按钮文本
   - 或者使用 [type="submit"]、button[type="submit"]
   - 避免使用 "button" 这样的模糊选择器
