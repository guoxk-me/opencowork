    ...
    try {
      const structure = await this.page.evaluate(() => {
        ...
        const sortedLinks = [...links].sort((a, b) => {
          ...
          const aIsExcluded = excludeKeywords.some(k => 
            aParent.includes(k) || aSelector.includes(k)
          );
          const bIsExcluded = excludeKeywords.some(k => 
            bParent.includes(k) || bSelector.includes(k)
          );
